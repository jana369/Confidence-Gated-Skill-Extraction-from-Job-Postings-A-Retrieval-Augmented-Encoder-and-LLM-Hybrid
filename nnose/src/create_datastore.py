...
class DataStore:
    ...
    def __init__(self, d=768, args=None):
        ...
        n_centroids = (
            4096  # number of clustering centroids to learn # 4096 was original
        )
        ...
        # else: keep the plain CPU index (faiss-cpu has no GPU cloning support)
    ...
    def train_index(self, key_store: np.ndarray) -> None:
        ...
        # else: index was trained on CPU already, nothing to move back
    ...
    def read_features_and_train(
        ...
        if self.args.whitening:
            ...
            # Only compare before/after when whitening actually ran, since
            # `old` (the pre-whitening snapshot) only exists in that case.
            old_t = torch.as_tensor(old[0][:10])
            new_t = (
                key_store[0][:10]
                if torch.is_tensor(key_store)
                else torch.as_tensor(key_store[0][:10])
            )
            print("Before:", old_t)
            print("After :", new_t)

            old_full = torch.as_tensor(old[0]).float()
            new_full = (
                key_store[0].float()
                if torch.is_tensor(key_store)
                else torch.as_tensor(key_store[0]).float()
            )
            print("Difference:", torch.norm(old_full - new_full).item())
    ...
    def search_k(self, query: torch.tensor, k: int, T: float) -> torch.tensor:
        ...
        distance_scores = torch.softmax(
            -torch.tensor(D, dtype=torch.float32) / T, dim=-1
        )  # softmax of the distance
        assert torch.isfinite(
            distance_scores
        ).all(), f"Invalid distance scores. D={D[:2]}, T={T}"
        # print("D:", D[:1])
        # print("distance_scores:", distance_scores[:1])
        # print("actual_label_ids:", actual_label_ids[:1])
        # print("label min:", actual_label_ids.min().item())
        # print("label max:", actual_label_ids.max().item())
        # print("vocab_size:", self.vocab_size)
        # print("scores finite:", torch.isfinite(distance_scores).all())
        ...
        return D, scores, actual_input_ids, I
...
def parse_args():
    ...
    parser.add_argument(
        "--temperature",
        type=str,
        default="1.0",  # must be > 0 (search_k divides by T); 0.0 would always raise
        help="Temperature for the softmax used when scoring kNN distances.",
    )
    parser.add_argument(
        "--whitening",
        action="store_true",

...
def main():
    ...
    if accelerator.is_main_process:
        if args.push_to_hub:
            if args.hub_model_id is None:
                ...
                repo_name = get_full_repo_name(
                    Path(args.output_dir).absolute().name, token=args.hub_token
                )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        current_f1 = float(eval_metric["f1"])

        # First epoch: always save
        if epoch == 0:
            best_eval_result = current_f1
            best_epoch = epoch
            ...
            logger.info(f"Epoch {epoch}: first checkpoint saved with F1={current_f1}")

        # Current epoch is better than previous best
        elif current_f1 > best_eval_result:
            ...
            logger.info(
                f"Epoch {epoch}: new best F1={current_f1} "
                f"(previous best={best_eval_result})"
            )
            ...
            old_checkpoint = os.path.join(
                args.output_dir,
                f"epoch_{old_best_epoch}"
            )

            if os.path.exists(old_checkpoint):
                shutil.rmtree(old_checkpoint, ignore_errors=True)
                logger.info(
                    f"Deleted previous checkpoint: {old_checkpoint}"
                )

        # Current epoch is worse
        else:

            patience += 1

            logger.info(
                f"Epoch {epoch}: F1={current_f1} "
                f"did not improve best F1={best_eval_result}"
            )
    ...
    logger.info(
        f"Best checkpoint (epoch {best_epoch}, F1={best_eval_result}) is available at: "
        f"{os.path.join(args.output_dir, f'epoch_{best_epoch}')}"
    )
